...
public class LinksV4 : GenerationTask
{
    ...
    public static void Register(IncrementalGeneratorInitializationContext context)
    {
        ...
        var actors = context.SyntaxProvider.CreateSyntaxProvider(
            ...
            .Where(x => x is not null)
            .Collect()
            .MixinSource(context, (context, actors) =>
            {
                GenerateAliases(context, actors);
                GenerateIdentities(context, actors);
            })
            .SelectMany((x, _) => x.Left.Select(y => y!.Update(x.Compilation)))
            .Where(x => x is not null)
            .Collect();
    ...
    private static void GenerateIdentities(MixinSourceProductionContext context, ImmutableArray<LinkTarget> actors)
    {
        var identities = actors
            .Where(x => !x.Actor.IsGenericType)
            .Select(x =>
                $"global using {GetFriendlyName(x.Actor)}Identity = Discord.IIdentifiable<{x.Id}, {x.Entity}, {x.Actor}, {x.Model}>;"
            );
    ...
    private static void GenerateAliases(MixinSourceProductionContext context, ImmutableArray<LinkTarget> targets)
    {
        var aliases = targets
            .Where(x => !x.Actor.IsGenericType)
            .Select(x =>
