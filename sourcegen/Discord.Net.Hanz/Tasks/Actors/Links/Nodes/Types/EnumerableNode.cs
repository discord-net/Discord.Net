...
public class EnumerableNode(LinkTarget target, LinkSchematics.Entry entry) : LinkTypeNode(target, entry)
{
    ...
    private protected override void Visit(NodeContext context, Logger logger)
    {
        ...
        RedefinesLinkMembers = Ancestors.Count > 0 || ExtraParameters.Count > 0;
    }
    ...
    protected override void AddMembers(List<string> members, NodeContext context, Logger logger)
    {
        ...
        if (ParentLinks.Any())
        {
            ...
            members.AddRange([
                $"ITask<IReadOnlyCollection<{Target.Entity}>> {Target.Actor}.{LinksV4.FormatTypeName(Entry.Symbol)}.AllAsync({FormatExtraParameters(ExtraParameters, defaults: false)}RequestOptions? options, CancellationToken token) => AllAsync({formattedInvocationParameters}options: options, token: token);"
            ]);
        }
    ...
    private void AddDefaultProvider()
    {
        ...
        RootActorNode.AdditionalTypes.AddRange([
            $"""
            internal static new Func<{(
                ExtraParameters.Count > 0
                    ? $"{string.Join(", ", ExtraParameters.Select(x => x.Type))}, "
                    : string.Empty
            )}{Target.Actor}.Enumerable, RequestOptions?, CancellationToken, ITask<IReadOnlyCollection<{Target.Entity}>>> DefaultEnumerableProvider = GetEnumerableProvider;
            """,
            $$"""
              internal static new async ITask<IReadOnlyCollection<{{Target.Entity}}>> GetEnumerableProvider(
