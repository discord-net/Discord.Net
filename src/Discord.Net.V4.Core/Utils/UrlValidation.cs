...
internal static class UrlValidation
{
    ...
    /// <param name="allowAttachments">
    ///     <see langword="true" /> to allow the <b>attachment://</b> protocol; otherwise
    ///     <see langword="false" />.
    /// </param>
    ...
    public static bool Validate(string url, bool allowAttachments = false)
    {
        ...
        if (!(url.StartsWith("http://", StringComparison.OrdinalIgnoreCase) ||
              url.StartsWith("https://", StringComparison.OrdinalIgnoreCase) || (allowAttachments
                  ? url.StartsWith("attachment://", StringComparison.Ordinal)
                  : false)))
            throw new InvalidOperationException(
                $"The url {url} must include a protocol (either {(allowAttachments ? "HTTP, HTTPS, or ATTACHMENT" : "HTTP or HTTPS")})");
        return true;
    }
    ...
    public static bool ValidateButton(string url)
    {
        ...
        if (!(url.StartsWith("http://", StringComparison.OrdinalIgnoreCase) ||
            ...
            throw new InvalidOperationException(
                $"The url {url} must include a protocol (either HTTP, HTTPS, or DISCORD)");
        return true;
    }
}
