...
public static class TokenUtils
{
    ...
    internal static string PadBase64String(string encodedBase64)
    {
        ...
        if (padding == 3)
            ...
            throw new FormatException(
                "The provided base64 string is corrupt, as it requires an invalid amount of padding.");
        else if (padding == 0)
            return encodedBase64;
    ...
    /// </summary>
    internal static char[] IllegalTokenCharacters = new char[] { ' ', '\t', '\r', '\n' };
    ...
    public static void ValidateToken(TokenType tokenType, string token)
    {
        ...
        if (string.IsNullOrWhiteSpace(token))
            throw new ArgumentNullException(paramName: nameof(token),
                message: "A token cannot be null, empty, or contain only whitespace.");
        // ensure that there are no whitespace or newline characters
        if (CheckContainsIllegalCharacters(token))
            throw new ArgumentException(
                message:
                "The token contains a whitespace or newline character. Ensure that the token has been properly trimmed.",
        ...
        switch (tokenType)
        {
            ...
            case TokenType.Bot:
                ...
                if (token.Length < MinBotTokenLength)
                    throw new ArgumentException(
                        message: $"A Bot token must be at least {MinBotTokenLength} characters in length. " +
