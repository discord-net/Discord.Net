    ...
    internal void AddComponent(IMessageComponent component, int row)
    {
        switch (component)
        {
            ...
            case SelectMenuComponent menu:
                WithSelectMenu(menu.CustomId, menu.Options?.Select(x => new SelectMenuOptionBuilder(x.Label, x.Value, x.Description, x.Emote, x.IsDefault)).ToList(), menu.Placeholder, menu.MinValues, menu.MaxValues, menu.IsDisabled, row, menu.Type, menu.ChannelTypes?.ToArray());
                break;
        }
    }
...
public class SelectMenuBuilder
{
    ...
    private List<SelectMenuOptionBuilder>? _options = new ();
    ...
    public SelectMenuBuilder(string customId, List<SelectMenuOptionBuilder>? options = null, string? placeholder = null, int maxValues = 1,
        int minValues = 1, bool isDisabled = false, ComponentType type = ComponentType.SelectMenu, List<ChannelType>? channelTypes = null)
    {
        CustomId = customId;
        Options = options;
    ...
    public SelectMenuComponent Build()
    {
        ...
        if (string.IsNullOrEmpty(CustomId))
            throw new ArgumentNullException("A select menu must have a custom id.");
...
public class SelectMenuOptionBuilder
{
    ...
    /// </summary>
    /// <exception cref="ArgumentOutOfRangeException" accessor="set"><see cref="Label"/> length exceeds <see cref="MaxSelectLabelLength"/></exception>
    /// <exception cref="ArgumentOutOfRangeException" accessor="set"><see cref="Label"/> length subceeds 1.</exception>
    public string? Label
    {
        get => _label;
        set => _label = value?.Length switch
    ...
    /// </summary>
    /// <exception cref="ArgumentOutOfRangeException" accessor="set"><see cref="Value"/> length exceeds <see cref="MaxSelectValueLength"/>.</exception>
    /// <exception cref="ArgumentOutOfRangeException" accessor="set"><see cref="Value"/> length subceeds 1.</exception>
    public string? Value
    {
        get => _value;
    ...
    public SelectMenuOption Build()
    {
        if (string.IsNullOrEmpty(Label))
            throw new ArgumentNullException(nameof(Label), "A select menu option must have a label.");
        if (string.IsNullOrEmpty(Value))
            throw new ArgumentNullException(nameof(Label), "A select menu option must have a value.");
