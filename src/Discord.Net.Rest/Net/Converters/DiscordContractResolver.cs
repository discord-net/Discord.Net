...
namespace Discord.Net.Converters
{
    internal class DiscordContractResolver : DefaultContractResolver
    {
        ...
        private static readonly MethodInfo _shouldSerialize =
            typeof(DiscordContractResolver).GetTypeInfo().GetDeclaredMethod("ShouldSerialize");
        ...
        protected override JsonProperty CreateProperty(MemberInfo member, MemberSerialization memberSerialization)
        {
            ...
            if (member is PropertyInfo propInfo)
            {
                ...
                throw new InvalidOperationException(
                    $"{member.DeclaringType.FullName}.{member.Name} is not a property.");
            ...
            if (type.IsConstructedGenericType)
            {
                ...
                if (depth == 0 && genericType == typeof(Optional<>))
                {
                    ...
                    var shouldSerializeDelegate =
                        (Func<object, Delegate, bool>)shouldSerialize.CreateDelegate(
                            typeof(Func<object, Delegate, bool>));
                    property.ShouldSerialize = x => shouldSerializeDelegate(x, getterDelegate);

                    return MakeGenericConverter(property, propInfo, typeof(OptionalConverter<>), innerTypeOutput,
                        depth);
                }

                if (genericType == typeof(Nullable<>))
                    return MakeGenericConverter(property, propInfo, typeof(NullableConverter<>),
                        type.GenericTypeArguments[0], depth);
                if (genericType == typeof(EntityOrId<>))
                    return MakeGenericConverter(property, propInfo, typeof(UInt64EntityOrIdConverter<>),
                        type.GenericTypeArguments[0], depth);
            }

            //Primitives
            var hasInt53 = propInfo.GetCustomAttribute<Int53Attribute>() != null;
            if (!hasInt53)
                if (type == typeof(ulong))
                    return UInt64Converter.Instance;
            var hasUnixStamp = propInfo.GetCustomAttribute<UnixTimestampAttribute>() != null;
            if (hasUnixStamp)
        ...
        private static bool ShouldSerialize<TOwner, TValue>(object owner, Delegate getter) =>
            (getter as Func<TOwner, Optional<TValue>>)((TOwner)owner).IsSpecified;

        private static JsonConverter MakeGenericConverter(JsonProperty property, PropertyInfo propInfo,
            Type converterType, Type innerType, int depth)
        {
