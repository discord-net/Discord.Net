...
namespace Discord
{
    public static class DiscordClientExtensions
    {
        ...
        public static async Task<IEnumerable<IDMChannel>> GetDMChannelsAsync(this IDiscordClient client)
            => (await client.GetPrivateChannelsAsync().ConfigureAwait(false)).Select(x => x as IDMChannel)
                .Where(x => x != null);
        ...
        public static async Task<IEnumerable<IGroupChannel>> GetGroupChannelsAsync(this IDiscordClient client)
            => (await client.GetPrivateChannelsAsync().ConfigureAwait(false)).Select(x => x as IGroupChannel)
                .Where(x => x != null);
