...
namespace Discord.WebSocket
{
        ...
        ISocketAudioChannel
    {
            ...
            : base(discord, id)
        {
            ...
            _voiceStates =
                new ConcurrentDictionary<ulong, SocketVoiceState>(ConcurrentHashSet.DefaultConcurrencyLevel, 5);
        ...
        public IReadOnlyCollection<SocketMessage> CachedMessages =>
            _messages?.Messages ?? ImmutableArray.Create<SocketMessage>();
        ...
        public IReadOnlyCollection<SocketMessage> GetCachedMessages(ulong fromMessageId, Direction dir,
        ...
        public IReadOnlyCollection<SocketMessage> GetCachedMessages(IMessage fromMessage, Direction dir,
            int limit = DiscordConfig.MaxMessagesPerBatch)
        ...
        {
            var users = new ConcurrentDictionary<ulong, SocketGroupUser>(ConcurrentHashSet.DefaultConcurrencyLevel,
                (int)(models.Length * 1.05));
        ...
        public IAsyncEnumerable<IReadOnlyCollection<IMessage>> GetMessagesAsync(
            ...
            => SocketChannelHelper.GetMessagesAsync(this, Discord, _messages, null, Direction.Before, limit,
                CacheMode.AllowDownload, options);
        ...
        {
            if (_users.TryGetValue(model.Id, out var user))
                return user;
