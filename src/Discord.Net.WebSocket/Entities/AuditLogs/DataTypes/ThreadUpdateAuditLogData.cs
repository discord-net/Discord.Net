...
/// </summary>
public class ThreadUpdateAuditLogData : ISocketAuditLogData
{
    private ThreadUpdateAuditLogData(ThreadType type, ThreadInfo before, ThreadInfo after)
    {
        ThreadType = type;
        Before = before;
        After = after;
    ...
    internal static ThreadUpdateAuditLogData Create(DiscordSocketClient discord, EntryModel entry)
    {
        var changes = entry.Changes;
            
        var (before, after) = AuditLogHelper.CreateAuditLogEntityInfo<ThreadInfoAuditLogModel>(changes, discord);

        return new ThreadUpdateAuditLogData(before.Type, new(before), new (after));
    }
