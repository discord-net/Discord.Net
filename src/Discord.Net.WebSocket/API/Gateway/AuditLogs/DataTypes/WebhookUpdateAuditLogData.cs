...
public class WebhookUpdateAuditLogData : IAuditLogData
{
    ...
    internal static WebhookUpdateAuditLogData Create(DiscordSocketClient discord, EntryModel entry)
    {
        var changes = entry.Changes;

        var (before, after) = AuditLogHelper.CreateAuditLogEntityInfo<Model>(changes, discord);

        return new WebhookUpdateAuditLogData(new(before), new(after));
    }
