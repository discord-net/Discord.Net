...
namespace Discord.WebSocket
{
    ...
    public class ChannelUpdateAuditLogData : IAuditLogData
    {
        ...
        internal static ChannelUpdateAuditLogData Create(DiscordSocketClient discord, EntryModel entry)
        {
            var changes = entry.Changes;

            var (before, after) = AuditLogHelper.CreateAuditLogEntityInfo<ChannelInfoAuditLogModel>(changes, discord);

            return new ChannelUpdateAuditLogData(entry.TargetId!.Value, new(before), new(after));
        }
