...
namespace Discord.Net
{
    public class HttpMixin : IAkavacheHttpMixin
    {
        ...
        public IObservable<byte[]> DownloadUrl(IBlobCache This, string url, IDictionary<string, string> headers = null,
            bool fetchAlways = false, DateTimeOffset? absoluteExpiration = null) =>
            This.DownloadUrl(url, url, headers, fetchAlways, absoluteExpiration);
        ...
        /// <param name="absoluteExpiration">An optional expiration date.</param>
        /// <returns>The data downloaded from the URL.</returns>
        public IObservable<byte[]> DownloadUrl(IBlobCache This, string key, string url,
            IDictionary<string, string> headers = null, bool fetchAlways = false,
            DateTimeOffset? absoluteExpiration = null)
        {
            var doFetch = MakeWebRequest(new Uri(url), headers)
                .SelectMany(x => ProcessWebResponse(x, url, absoluteExpiration));
            var fetchAndCache = doFetch.SelectMany(x => This.Insert(key, x, absoluteExpiration).Select(_ => x));

            var ret = !fetchAlways ? This.Get(key).Catch(fetchAndCache) : fetchAndCache;
        ...
        private IObservable<byte[]> ProcessWebResponse(WebResponse wr, string url, DateTimeOffset? absoluteExpiration)
        {
            ...
            if ((int)hwr.StatusCode >= 400) return Observable.Throw<byte[]>(new WebException(hwr.StatusDescription));
        ...
        private static IObservable<Unit> WriteAsyncRx(Stream stream, byte[] data, int start, int length) =>
            stream.WriteAsync(data, start, length).ToObservable();
    }
}
